...
for(seed in 8) {
  ...
  make_circle <- function(n = 10) {
    angle <- sort(runif(n - 1, min = 0, max = 2*pi))
    angle <- c(angle, angle[1])
  ...
  make_object <- function(base, shade, recurse = 4, stdev = .1,
                          layers = 50, scale = NULL, elevation = NULL) {
    xsh <- 0 #runif(1, -1 , 1)
    ysh <- 0 #runif(1, -1 , 1)
  ...
  droplet <- function(shade, index, size = .15, scatter = 1) {

    x_shift <- rnorm(1, 0, scatter)
    y_shift <- rnorm(1, 0, scatter)
    sc_shift <- pnorm(abs(x_shift), 0, scatter) *
      pnorm(abs(y_shift), 0, scatter)
    el_shift <- runif(1)
    layers <- 5

    base_shape <- make_circle(n = 12) %>%
      make_base_shape(recurse = 6, stdev = .3)

    base_size <- size + runif(1, -size * .75, size * .75)

    inner <- base_shape %>%
      make_object(
        shade = shade,
        scale = (1 - sc_shift) * base_size,
        layers = layers,
        elevation = (index * layers) + el_shift + 0.5,
        stdev = .2,
        recurse = 5
      ) %>%
      mutate(
  ...
  dat <- shades %>%
   sample(size = 50, replace = TRUE) %>%
    imap_dfr(droplet, size = .3, scatter = .5)

  cat("  render step\n")

  border <- 1.3
  pic <- ggplot(dat, aes(x, y, group = factor(group), fill = shade)) +
    geom_polygon(alpha = .4, show.legend = FALSE) +
    coord_equal(
      xlim = c(-1, 1) * border,
      ylim = c(-1, 1) * border
    ) +
