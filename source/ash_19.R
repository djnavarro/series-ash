...
for(seed in 2) {
  ...
  make_blob <- function(x, circ) {
    circ %>%
      make_base_shape(stdev = 1, recurse = 5) %>%
      make_blurred_shape(stdev = .4, layers = 20, recurse = 5) %>%
      mutate(group = id + runif(1, 0, .1))  %>%
      mutate(x = x * .5, y = y * .5) %>%
      mutate(shade = "white")
  }

  circ <- make_circle(n = 12)
  dat <- map_dfr(1:2, make_blob, circ = circ)
