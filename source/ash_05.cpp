...
double deform(
  ...
  //for(int d = 0; d < depth; ++d) {
    ...
    for(int i = 0; i < (npt - 1); ++i) {
      ...
      seg_len = (x_diff * x_diff) + (y_diff * y_diff); // euclidean distance
      //seg_len = sqrt(seg_len);

      // generate insertion points
      x_insert = x_mid + drift_mean[i]; //Rf_rnorm(0, sigma)
      y_insert = y_mid + drift_mean[i]; //Rf_rnorm(0, sigma)
  ...
  //cpp11::writable::doubles_matrix out(10, 2);
  //for(int i = 0; i < npt; ++i) {
  //  out(i, 0) = old_x[i];
  //  out(i, 1) = old_y[i];
  //}
