...
for(seed in 1:20) {
  ...
  droplet <- function(shade, index, size = .15, scatter = 1) {
    ...
    sz_shift <- rbeta(1, 1, 15) * size * 1.5 - size * .75
    el_shift <- runif(1)
    layers <- 10
    make_circle(n = 12) %>%
      make_base_shape(recurse = 2, stdev = .3) %>%
      make_object(
        shade = shade,
        scale = (1 - sc_shift) * (size + sz_shift),
        layers = layers,
  ...
  dat <- shades %>%
   sample(size = 300, replace = TRUE) %>%
    imap_dfr(droplet, size = .3, scatter = .6)
